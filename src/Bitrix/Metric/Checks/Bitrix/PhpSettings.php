    ...
    public function run()
    {
        ...
        foreach ($arRequiredParams as $param => $val) {
            ...
            if ($cur != $val) {
                $result = false;
                $this->logError(
                    sprintf('Параметр %s = %s, требуется %s', $param, $cur ? htmlspecialcharsbx($cur) : 'off', $val ? 'on' : 'off')
                );
            }
        }
        ...
        if (($vm = getenv('BITRIX_VA_VER')) && version_compare($vm, '4.2.0', '<')) {
            $result = false;
            $this->logError(
                'Вы используете Битрикс веб-окружение старой версии, установите актуальную версию чтобы не было проблем с настройкой сервера.'
            );
        }
